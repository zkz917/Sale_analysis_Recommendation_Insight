...
public class RecommenderListGenerator {
	public static class RecommenderListGeneratorMapper extends Mapper<LongWritable, Text, IntWritable, Text> {
		...
		public void map(LongWritable key, Text value, Context context) throws IOException, InterruptedException {
			//recommender user \t product:rating
			String[] tokens = value.toString().split("\t");
			int user = Integer.parseInt(tokens[0]);
			int product = Integer.parseInt(tokens[1].split(":")[0]);
			context.write(new IntWritable(user), new Text(product + ":" + tokens[1].split(":")[1]));
		}
	}
	...
	public static class RecommenderListGeneratorReducer extends Reducer<IntWritable, Text, IntWritable, Text> {
		...
		public void reduce(IntWritable key, Iterable<Text> values, Context context)
			...
			// Top K: recommend top k products with highest calculated ratings for each user
			int K = 5;
			PriorityQueue<product> heap = new PriorityQueue<product>();
			...
			while(values.iterator().hasNext()) {
				...
				if(heap.size() < K) {
					heap.offer(new product(product_id, rating));
				} else {
			...
			while(!heap.isEmpty()) {
				product product = heap.poll();
				context.write(key, new Text(product.getproductId() + ":" + product.getRating()));
			}
		}
	}
