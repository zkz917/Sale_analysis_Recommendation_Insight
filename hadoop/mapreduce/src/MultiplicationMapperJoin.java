...
public class MultiplicationMapperJoin {
    ...
    public static class CoOccurrenceMapper extends Mapper<LongWritable, Text, IntWritable, Text> {
        ...
        public void map(LongWritable key, Text value, Context context) throws IOException, InterruptedException {
            //input productA: productB \t normalizedRelation
            //output productA \t coOccurrence:productB:normalizedRelation
            ...
            context.write(new IntWritable(product1), new Text("coOccurrence:" + product2 + ":" + relation));
        }
    }
    ...
    public static class RateMapper extends Mapper<LongWritable, Text, IntWritable, Text> {
        ...
        @Override
        public void map(LongWritable key, Text value, Context context) throws IOException, InterruptedException {
            //input user,product,rating
            //output product \t rate:user:rating
            String[] tokens = value.toString().trim().split(",");
            int user = Integer.parseInt(tokens[0]);
            int product = Integer.parseInt(tokens[1]);
            double rating = Double.parseDouble(tokens[2]);

            context.write(new IntWritable(product), new Text("rate:" + user + ":" + rating));
        }
    }
        ...
        public void reduce(IntWritable key, Iterable<Text> values, Context context)
            ...
            while(values.iterator().hasNext()) {
                ...
                if(tokens[0].equals("coOccurrence")) {
                    productRelationMap.put(Integer.valueOf(tokens[1]), Double.valueOf(tokens[2]));
                } else {
            ...
            for(Map.Entry<Integer, Double> ratingEntry: ratingMap.entrySet()) {
                ...
                for(Map.Entry<Integer, Double> relationEntry: productRelationMap.entrySet()) {
